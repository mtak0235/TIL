...
### JdbcTemplate과 트랜잭션 동기화
 ...
 만약 미리 생성돼서 트랜잭션 동기화 저장소에 등록된 DB 커넥션이나 트랜잭션이 없는 경우에는 
...
😎참고로  transaction을 JTA를 이용하고 싶다면  이 친구는 application server의 transaction srevice를 이용하기 때문에 직접  dataSource와 연동할 필요 없다. 즉, 서버에서 제공해주는 dataSource를 사용해야 한다. 
...
이제 스프링의 트랜잭션 서비스 추상화 기법을 이용해 
설정을 고치는 것 만으로 DB 연결 기술 , 데이터 접근 기술, 트랜잭션 기술을 자유롭게 바꿔 끼울 수 있다. 

### 수직 , 수평 계층 구조와 의존 관계

UserDao, UserService는 둘 다 application logic이지만, 기능적인 관심에 따라 분리했다. 
➡application logic 종류에 따른 수평적인 분리

트랜잭션이 추상화는 application의 business logic과 그 하위에서 동작하는 transaction tech라는 다른 계층의  특성을 가지는 코드를 분리했다. 
➡로직과 기술로 나눈, 수직적인 분리
...
🤔Um.. 개발 중이라 메일  서버가 준비되어 있지 않은 뎁쇼?
물론, 메일 서버가 있다면 당연히 테스트는 pass할거다.
근데, 메일 발송은 부하가 큰 작업이다. 그걸 매 테스트마다 한다고?!?

😎실제로 메일을 보내지는 않는 테스트용 메일 서버를 사용하면 어떨까?
어차피 메일 발송용 서버에 문제 없이 전달된 것만 확인하면 되잖아!

JavaMail도 smtp처럼 이미 검증된 모듈이니까 테스트 할 때 마다 직접 구동할 필요가 없다. 고로 JavaMail와 동일한 인터페이스를 가진 코드가 테스트때 돌아가게 만들면 된다!
...
클래스인 Session은 생성자가 죄다 private이라 static factory method로 밖에 obj를 만들 수 밖에 없고, 심지어 final class라 상속 마저 안된다. 
...
스프링의 예외처리  원칙에 따라서 
JavaMail을 처리하는 중에 발생한 각종 예외를 MailException이라는 런타임 예외로 포장해서 던져주기 때문에 
귀찮은 try/catch 블록을 만들지 않아도 된다.

😎자, 어쨌든 지저분한 JavaMailAPI를 썼을 때 비해 코드가 간결해졌다. 
하지만 여전히 JavaMailApi를 사용하지 않는 테스트용 obj를 대체할 순 없다.
DI로 MailSender 를 빼보자. 
...
메일 발송 기능이 포함된 코드를 JavaMail사용 없이 테스트 하기 위한 모든 준비가 끝났다.
메일 전송 기능을 추상화해 인터페이스를 적용하고 DI로 따로 분리해놨다 후후
...
또 메일을 바로 발송하지 않고 작업 큐에 담아뒀다가 정해진 시간에 메일을 발송하려면 MailSender를 구현한 메시지 발송 큐의 구현을 하나 만들어두고 다시 DI로  JavaMailServiceImpl같은 실제 메일 발송용 obj를 연결해서 사용하면 된다.

🤔level upgrade중 예외가 생겨서 DB를 롤백 됐는데 메일은 이미 발송해버렸다.즉, 트랜잭션이 빠져있네? 두 가지 솔루션이 있다. 

1. 메일을 업그레이드 할 사용자를 발견했을 때마다 발송하지 않고, 
   발송 대상을 별도의 목록에 저장해 두자.
   그리고 업그레이드 작업이 모두 성공적으로 끝났을 때 한번에 메일을 전송하는거다!
   🤔메일 저장용 리스트 등을 파라미터로 계속 가지고 다녀야 하네..?
2. MailSender를 확장해서 transaction  기능이 있는 메일 전송용 클래스를 만든다. 
   이 Obj에 업그레이드 작업 이전에 새로운 메일 전송 작업 시작을 알려주고, 그 때부터 mailSender.send()를 호출해도 실제 메일을 발송하지 않고 저장해둔다.
   업그레이드 작업이  끝나면 transaction기능을 가진 MailSender에 지금까지 저장된 메일을 모두 발송하고, 예외가 발생하면 모두 취소하게 할 수 있다. 
...
🤔테스트 대상 obj의 메소드 결과만 검증하는게 아니라,
의존 obj에 넘기는 값과 그 행위 자체에 대해서도 검증하고 싶다면 어떻게 해야할까?
...
하지만, 사용자 레벨 업그레이드 결과를 확인하는 upgradeLevels()테스트는 메일 전송 자체에도 관심이 있다. 고로 테스트 대상이 넘겨주는 출력ㄱ밧을 보관하는 기능이 추가된 MailSender 구현체를 만들어보자. 
