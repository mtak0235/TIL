...
public class ReflectionTest2 {
    ...
    public void getConstructor() throws ClassNotFoundException, NoSuchMethodException {
        ...
        assertThat(publicConstructorsOfParentNChild.stream().filter(x -> !(x.toString().contains("public"))).count()).isEqualTo(0);
    }
    ...
    public void getMethod() throws ClassNotFoundException, NoSuchMethodException {
        ...
        assertThat(method5.toString()).isEqualTo("private int com.example.tobby.Child.method5(int,java.lang.String)");
    }

    @Test
    @DisplayName("Class 정보 중 Field 변경")
    public void getField() throws ClassNotFoundException, NoSuchFieldException, InstantiationException, IllegalAccessException {
        Class<?> clazz = Class.forName("com.example.tobby.Child");
        ...
        assertThat(Arrays.stream(fields).filter(x -> !x.toString().contains("public")).count() > 0).isEqualTo(false);
